        ...
        for inst in text_instances:
            ...
            if 'lines' in inst:  # Ensure we are looking at text
                for line in inst['lines']:
                    ...
                    for span in spans:
                        text = span['text']
                        if re.search(currency_pattern, text) and not re.search(date_pattern, text):
                            # Redact currency amounts not part of a date
                            rect = fitz.Rect(span['bbox'])
                            page.add_redact_annot(rect, text=" ")
                        elif transaction_details in text:
                            # Start of transaction details section
                            transaction_start = True
                        elif transaction_start and not any(keyword.lower() in text.lower() for keyword in keep_keywords):
...
keep_keywords = ['TFL', 'Travel', 'Oyster', 'Trainline']
